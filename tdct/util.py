...
### GAUSSIAN FITTING ###
def get_z_gauss(image: np.ndarray, x: int, y: int, show: bool = False) -> float:
    ...
    if x >= image.shape[-1] or y >= image.shape[-2]:
        raise ValueError(f"x and y must be within the image shape, x: {x}, y: {y}, {image.shape}")
    ...
    zval, zidx, zsigma = poptZ # unpack the optimal parameters
...
def interpolate_z_stack(image: np.ndarray, pixelsize_in: float, pixelsize_out: float, method: str = 'linear') -> np.ndarray:
    return interpol(image, pixelsize_in, pixelsize_out, method)

def interpol(img: np.ndarray, ss_in: float, ss_out: float, method: str = "linear") -> np.ndarray:
    ...
    if img.ndim == 4:
        if img.shape[0] != 1:
            raise ValueError(f"Muli-channel images are not supported, got an image of shape {img.shape}, expected (1, Z, Y, X) or (Z, Y, X)")
    ...
    if method not in INTERPOLATION_METHODS:
        raise ValueError(f"interpolation method  must be in {INTERPOLATION_METHODS} got {method}")
...
def get_interpolated_shape(image: np.ndarray, ss_in: float, ss_out: float) -> Tuple[int, int, int]:
    ...
    sl_out = int((sl_in-1)*(ss_in/ss_out)) + 1      # Number of slices in interpolated stack
...
def fast_cubic_interpolate(image_3d: np.ndarray, original_z_size: float, target_z_size: float) -> np.ndarray:
    """
    ...
    interpolated = ndimage.zoom(image_3d, 
                       zoom_factors, 
                       order=3,
                       mode='reflect',
                       prefilter=True)
