...
'''Calculate projection of 3D volumes based on 3DCT parameters

Call GUI via wrapper function fluo_project_GUI or from command line.
Run 'python3 project_fluo.py --help' command line usage information.

Multiple 3D volumes and parameter sets can be specified.
An instance of the Projector class is created for each combination of volume and parameter set.
Affine transform and projection of the volume are performed with class method fluorescence_proj_mt.
The calculation is multi-threaded using PyQt QThreads, with each thread handling one patch of the projection.
For each patch, the z-range of the transformed volume to be evaulated is determined to reduce computation.
...
from tdct.tools3dct.core import Param3D, corr_transform
from tdct.tools3dct import docs
